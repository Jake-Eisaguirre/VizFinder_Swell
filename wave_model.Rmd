...
#   geom_raster(aes(y=y, x=x, 
#                   fill = height, group = date), interpolate = T) +
#   scale_fill_gradientn(colours = c("slategray1","skyblue", 
#                                    "royalblue1", "purple1", "firebrick1", "firebrick4")) +
#   theme_classic() +
#   theme(axis.line=element_blank(),axis.text.x=element_blank(),
...
# pal
```{r}
pal <- sequential_hcl(n = 100, h = c(308, 51), c = c(85, 162, 43), l = c(22, 100), power = c(1.1, 1.3), rev = TRUE, register = )
```

#loop to create each plot 
```{r}
swell.list = lapply(sort(unique(clean_data$date)), function(i) {
  ggplot(clean_data[clean_data$date==i,], aes(y=y, x=x,
                                              fill = height)) +
