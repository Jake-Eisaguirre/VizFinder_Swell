...
# pal
```{r}
pal <- colorRampPalette(c("lightskyblue2", "deepskyblue2", "deepskyblue4", "seagreen3", "palegreen1", "khaki2", "orangered",
                          "darkred"))
limits <- c(0, 16)
```
...
swell.list = lapply(sort(unique(clean_data$date)), function(i) {
  ...
  scale_fill_gradientn(colours = pal(100), limits = limits) +
  theme_classic() +
